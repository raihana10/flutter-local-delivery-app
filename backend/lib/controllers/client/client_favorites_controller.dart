...
class ClientFavoritesController {
  ...
  Future<Response> addFavorite(Request request) async {
    ...
    try {
      ...
      if (idBusiness == null) {
        return Response(
          400,
          body: jsonEncode({'error': 'id_business is required'}),
          headers: {'content-type': 'application/json'},
        );
      }

      final newFavoris = await SupabaseConfig.client
          .from('favoris')
          .insert({'id_client': clientId, 'id_business': idBusiness})
          .select()
          .single();

      return Response.ok(
        jsonEncode({'data': newFavoris}),
        headers: {'content-type': 'application/json'},
      );
    } catch (e) {
      return Response(
  ...
  Future<Response> removeFavorite(Request request, String idBusiness) async {
    ...
    try {
      ...
      return Response.ok(
        jsonEncode({'message': 'Favorite removed successfully'}),
        headers: {'content-type': 'application/json'},
      );
    } catch (e) {
      return Response(
