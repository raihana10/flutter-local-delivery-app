...
class _OrdersManagementScreenState extends State<OrdersManagementScreen> {
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      case 'confirmee': return AppColors.accent;
      case 'preparee': return const Color(0xFFF57C00);
      case 'en_livraison': return const Color(0xFF1976D2);
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Voulez-vous vraiment rembourser le client ${order['client']} ?', style: const TextStyle(fontWeight: FontWeight.bold)),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  const Text('Montant à rembourser:', style: TextStyle(color: AppColors.mutedForeground)),
                  Text('${order['prix_donne']} MAD', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                ],
              ),
            ),
            const SizedBox(height: 12),
            const Text('Cette action est irréversible et passera le montant payé à 0 MAD.', style: TextStyle(fontSize: 12, color: AppColors.mutedForeground)),
          ],
        ),
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              setState(() {
                final index = orders.indexWhere((o) => o['id_commande'] == order['id_commande']);
                if (index != -1) {
                  orders[index] = Map<String, dynamic>.from(orders[index]);
                  orders[index]['prix_donne'] = 0.0;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Commande #${order['id_commande']} remboursée avec succès.'), backgroundColor: Colors.green),
                  );
                }
              });
            },
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Commande #${order['id_commande']}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  IconButton(
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _DetailItem(icon: LucideIcons.user, label: 'Client', value: order['client']),
                        const SizedBox(height: 12),
                        _DetailItem(icon: LucideIcons.store, label: 'Restaurant/Boutique', value: order['business']),
                        const SizedBox(height: 12),
                        _DetailItem(icon: LucideIcons.bike, label: 'Livreur', value: order['livreur'] ?? 'En attente d\'assignation'),
                      ],
                    ),
                  ),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        _DetailItem(icon: LucideIcons.calendar, label: 'Date', value: (order['date'] as String).replaceFirst('T', ' ').substring(0, 16)),
                        const SizedBox(height: 12),
                        _DetailItem(icon: LucideIcons.tag, label: 'Type', value: order['type']),
                        const SizedBox(height: 12),
                        _DetailItem(icon: LucideIcons.creditCard, label: 'Total Payé', value: '${order['prix_donne']} MAD'),
                      ],
                    ),
                  )
                ],
              ...
              Container(
                 ...
                 child: Row(
                   ...
                   children: [
                     Icon(LucideIcons.package2, color: _getStatusColor(order['statut'])),
                     const SizedBox(width: 8),
                     Text(
                       _formatStatus(order['statut']).toUpperCase(),
                       style: TextStyle(color: _getStatusColor(order['statut']), fontWeight: FontWeight.bold, letterSpacing: 1.2),
                     )
                   ],
                 ),
              ),
...
class _OrderDataTableSource extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(
      cells: [
        DataCell(Text('#${order['id_commande']}', style: const TextStyle(fontWeight: FontWeight.bold))),
        DataCell(Text(order['client'])),
        DataCell(Text(order['business'])),
        DataCell(Text(order['livreur'] ?? 'Non assigné', style: TextStyle(color: order['livreur'] == null ? AppColors.mutedForeground : AppColors.foreground, fontStyle: order['livreur'] == null ? FontStyle.italic : FontStyle.normal))),
        DataCell(Text('${order['prix_donne']} MAD', style: const TextStyle(fontWeight: FontWeight.bold))),
        DataCell(
          Container(
          ...
          Row(
            children: [
              ...
              if (order['prix_donne'] > 0)
                IconButton(
                  icon: const Icon(LucideIcons.banknote, color: AppColors.destructive, size: 20),
                  tooltip: 'Rembourser',
