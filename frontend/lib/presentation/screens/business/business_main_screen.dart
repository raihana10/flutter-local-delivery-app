            ...
            children: [
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: isOpen ? AppColors.amber : Colors.white.withOpacity(0.1),
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final orders = [
                    {'id': '#1042', 'client': 'Alae B.', 'items': 3, 'time': '5 min'},
                    {'id': '#1043', 'client': 'Sara M.', 'items': 2, 'time': '2 min'},
                  ];
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Row(
                              children: [
                                Text('${o['id']}', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.forest)),
                                const SizedBox(width: 8),
                                Text('${o['client']}', style: const TextStyle(color: AppColors.mutedForeground, fontSize: 12)),
                              ],
                            ),
                            Text('${o['time']}', style: const TextStyle(color: AppColors.gold, fontWeight: FontWeight.bold, fontSize: 12)),
                          ],
                        ),
                        const SizedBox(height: 4),
                        Text('${o['items']} articles', style: const TextStyle(color: AppColors.mutedForeground, fontSize: 12)),
                        const SizedBox(height: 12),
                        Row(
                          children: [
                            _buildOrderButton('Accepter', AppColors.forest, Colors.white),
                            const SizedBox(width: 8),
                            _buildOrderButton('Préparer', AppColors.sage, Colors.white),
                            const SizedBox(width: 8),
                            _buildOrderButton('Prêt', AppColors.amber, AppColors.forest),
                          ],
                        ),
                      ],
                    ),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            const Text('Ajouter un produit', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: AppColors.forest)),
            ...
            ListTile(
              ...
              title: const Text('Ajout manuel', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            ListTile(
              leading: Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(color: AppColors.warmWhite, borderRadius: BorderRadius.circular(12)),
                child: const Icon(LucideIcons.fileSpreadsheet, color: AppColors.forest),
              ),
              title: const Text('Import par fichier', style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        Column(
          ...
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  const Text('Catalogue', style: TextStyle(color: AppColors.forest, fontWeight: FontWeight.bold, fontSize: 20)),
                ],
              ),
            ),
            ...
            SingleChildScrollView(
              ...
              padding: const EdgeInsets.symmetric(horizontal: 24),
              child: Row(
                children: ['Tout', 'Plats', 'Entrées', 'Boissons'].asMap().entries.map((e) {
                  ...
                  return Container(
                    ...
                    child: Text(
                      e.value,
                      style: TextStyle(
                        color: isFirst ? Colors.white : AppColors.mutedForeground,
                        fontWeight: FontWeight.bold,
                        fontSize: 12,
                      ),
                    ),
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      GestureDetector(
                                        onTap: () => onNavigate(BusinessScreen.editProduct, index: index),
        ...
        Positioned(
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: const Icon(LucideIcons.plus, color: AppColors.forest, size: 24),
            ),
          ),
        ),
      ],
...
class _ImportSheetViewState extends State<_ImportSheetView> {
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 12),
              const Text('Import par fichier', style: TextStyle(color: AppColors.forest, fontWeight: FontWeight.bold, fontSize: 20)),
            ],
          ),
        ),
        ...
        Expanded(
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 24),
            child: _previewItems.isEmpty ? _buildUploadStep() : _buildPreviewStep(),
          ),
        ),
      ],
    );
    ...
    return Center(
      child: Column(
        ...
        children: [
          Container(
            padding: const EdgeInsets.all(32),
            decoration: BoxDecoration(color: AppColors.warmWhite, borderRadius: BorderRadius.circular(32)),
            child: const Icon(LucideIcons.fileSpreadsheet, size: 48, color: AppColors.forest),
          ),
          const SizedBox(height: 24),
          const Text('Sélectionnez un fichier', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          const Text('CSV ou Excel (.xlsx)', style: TextStyle(color: AppColors.mutedForeground)),
          const SizedBox(height: 32),
          ElevatedButton(
            onPressed: _isLoading ? null : _pickFile,
            child: _isLoading ? const CircularProgressIndicator(color: Colors.white) : const Text('Choisir un fichier'),
          ),
        ],
      ),
    );
  ...
  Widget _buildPreviewStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Fichier : $_fileName', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.gold)),
        const SizedBox(height: 8),
        Text('${_previewItems.length} produits extraits', style: const TextStyle(color: AppColors.mutedForeground, fontSize: 12)),
        const SizedBox(height: 16),
        Expanded(
...
class _AddProductViewState extends State<_AddProductView> {
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                const Text('Nouveau produit', style: TextStyle(color: AppColors.forest, fontWeight: FontWeight.bold, fontSize: 20)),
              ],
            ),
          ),
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: const Column(
                    children: [
                      Icon(LucideIcons.camera, color: AppColors.forest, size: 32),
                      SizedBox(height: 8),
                      Text('Ajouter une photo', style: TextStyle(color: AppColors.mutedForeground, fontSize: 13)),
                    ],
                  ),
                ),
                const SizedBox(height: 20),
                
                _buildField('Nom du produit', 'Ex: Tajine Berbère', controller: _nameController),
                const SizedBox(height: 16),
                _buildField('Description', 'Détails du produit...', lines: 2, controller: _descController),
                const SizedBox(height: 16),
                _buildField('Prix (MAD)', '0.00', keyboardType: TextInputType.number, controller: _priceController),
                const SizedBox(height: 20),

                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('Disponible', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.forest)),
                    Switch(
                      value: _isDispo, 
  ...
  Widget _buildField(String label, String placeholder, {int lines = 1, TextInputType? keyboardType, TextEditingController? controller}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppColors.forest)),
        const SizedBox(height: 8),
        TextField(
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                const Text('Modifier le produit', style: TextStyle(color: AppColors.forest, fontWeight: FontWeight.bold, fontSize: 20)),
              ],
            ),
          ),
            ...
            child: Row(
              children: [
                ...
                const Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Commande #1042', style: TextStyle(color: AppColors.forest, fontWeight: FontWeight.bold, fontSize: 18)),
                    Text('Il y a 5 min', style: TextStyle(color: AppColors.mutedForeground, fontSize: 12)),
                  ],
                ),
                const Spacer(),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(color: AppColors.amber.withOpacity(0.2), borderRadius: BorderRadius.circular(20)),
                  child: const Text('En cours', style: TextStyle(color: AppColors.amber, fontSize: 12, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
          ),
                  ...
                  child: Row(
                    children: [
                      ClipRRect(
                        ...
                        child: Image.network('https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=80&h=80&fit=crop', width: 44, height: 44, fit: BoxFit.cover),
                      ),
                      const SizedBox(width: 12),
                      const Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Alae Benchakroun', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: AppColors.forest)),
                            Text('23, Rue Al Andalous', style: TextStyle(color: AppColors.mutedForeground, fontSize: 12)),
                          ],
                        ),
                      ),
                      Container(
                        ...
                        child: const Icon(LucideIcons.phone, color: AppColors.sage, size: 16),
                      ),
                    ],
                  ),
                ),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      _buildStep('Prêt pour livraison', false, true, isCurrent: true),
      ...
      child: Row(
        children: [
          Container(
            ...
            alignment: Alignment.center,
            child: Text('$qty', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: AppColors.forest)),
          ),
          const SizedBox(width: 12),
          Expanded(child: Text(name, style: const TextStyle(color: AppColors.forest, fontWeight: FontWeight.w500, fontSize: 14))),
          Text('$price MAD', style: const TextStyle(color: AppColors.gold, fontWeight: FontWeight.bold, fontSize: 14)),
        ],
      ),
    );
  }

  Widget _buildStep(String label, bool isDone, bool showLine, {bool isCurrent = false}) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Column(
          children: [
            Container(
              width: 24,
              height: 24,
              decoration: BoxDecoration(color: isDone ? AppColors.sage : (isCurrent ? AppColors.amber : AppColors.warmWhite), shape: BoxShape.circle),
              child: isDone ? const Icon(LucideIcons.check, color: Colors.white, size: 12) : null,
            ),
            if (showLine) Container(width: 2, height: 24, color: isDone ? AppColors.sage : Colors.black12),
          ],
        ),
        const SizedBox(width: 12),
        Padding(
          padding: const EdgeInsets.only(top: 3),
          child: Text(label, style: TextStyle(color: isDone ? AppColors.sage : AppColors.mutedForeground, fontWeight: isDone || isCurrent ? FontWeight.bold : FontWeight.normal, fontSize: 13)),
        ),
      ],
    );
  }
...
class _PulseIndicatorState extends State<_PulseIndicator> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    return FadeTransition(opacity: _controller, child: Container(width: 8, height: 8, decoration: const BoxDecoration(color: AppColors.destructive, shape: BoxShape.circle)));
  }
}
